...
class App extends Component {
    ...
    verifyProof = async (identity) => {
        ...
        const chain = chains.find((id) =>
            ChainId.from(id).equals(proof.chainId)
        )
        if (!chain) {
        ...
        const auth = account.getPermission(
            proof.signer.permission
        ).required_auth
    ...
    addAccount = async () => {
        ...
        console.log(JSON.parse(JSON.stringify(identity.resolvedTransaction)))
        ...
        const sessions = await this.link.listSessions(
    ...
    */
    ...
    signTransaction = async () => {
        ...
        try {
            ...
            const action = {
                ...
                name: 'approve',
                data: {
                    proposer: 'supportacc1c',
                    proposal_name: 'liwxwg',
                    level: {
                        actor: 'supportacc1c',
                        permission: 'active',
                    },
    ...
    removeSession = async (ls: LinkSession) => {
        ...
        if (
            session &&
            session.auth.actor.equals(ls.auth.actor) &&
            session.auth.permission.equals(ls.auth.permission)
        ) {
        ...
        return (
            <Container style={{ paddingTop: '2em' }}>
                <Header attached="top" block size="huge">
                    ...
                    <Header.Subheader>
                        <p>
                            An anchor-link demo that allows multiple persistent
                            logins from different blockchains and accounts.
                        </p>
                        <p>
                            Source code:{' '}
                            <a href="https://github.com/greymass/anchor-link-demo-multipass">
                ...
                <Segment attached padded>
                    <Header>
                        Account Data
                        <Header.Subheader>
                            Example rendering data from API client call
                        </Header.Subheader>
                    </Header>
                    {account && account.cpu_limit ? (
                        <p>
                            CPU: {String(account.cpu_limit.available)}μs /{' '}
                            {String(account.cpu_limit.max)}μs
                        </p>
                    ) : (
                ...
                <Segment attached padded textAlign="center">
                    ...
                    {!session ? (
                        <p style={{ marginTop: '0.5em' }}>
                            Login using anchor-link to sign a test transaction.
                        </p>
                    ) : (
                        false
                    )}
                ...
                <Segment attached="bottom" padded>
                    {error ? <SigningError error={error} /> : false}
